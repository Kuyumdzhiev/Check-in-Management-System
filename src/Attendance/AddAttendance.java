...
public class AddAttendance {
    ...
    public void addView() throws SQLException {
        ...
        dtbox.setFont(text);
        dtbox.setForeground(Color.decode("#124b70"));
        String dateInString =new SimpleDateFormat("yyyy-MM-dd").format(new Date());
        dtbox.setText(dateInString);
        frame.add(dtbox);

        //-TEAM-
        JLabel teamss = new JLabel("TEAM : ");
        teamss.setFont(text);
        teamss.setBounds(25, 150, 100, 20);
        teamss.setForeground(Color.decode("#DEE4E7"));
        frame.add(teamss);
        @SuppressWarnings("unchecked")
        JComboBox tmss= new JComboBox(teamEt());
        tmss.setBounds(110, 150, 50, 25);
        frame.add(tmss);
        ...
        view.setForeground(Color.decode("#124b70"));
        frame.add(view);
        view.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                try {
                    if(check(String.valueOf(tmss.getSelectedItem()), dtbox.getText()))
                        txt.setText("Attendance Already marked!!!");
                    else
                        tblupdt(String.valueOf(tmss.getSelectedItem()));
        ...
        ab.setForeground(Color.decode("#124b70"));
        frame.add(ab);
        ...
        pre.setForeground(Color.decode("#124b70"));
        frame.add(pre);
        ...
        sbmt.setForeground(Color.decode("#124b70"));
        frame.add(sbmt);
        sbmt.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                for(int i= 0; i<table.getRowCount(); i++) {
                    try {
                        addItem(Integer.parseInt(String.valueOf(table.getValueAt(i, 0))), String.valueOf(table.getValueAt(i, 2)),
                                dtbox.getText(), String.valueOf(tmss.getSelectedItem()));
    ...
    public ResultSet dbSearch(String teams) throws SQLException {
        String str1 = "SELECT * from employees where class = '"+teams+"'";
    ...
    public void tblupdt(String teams) {
        for (int i=0; i < model.getRowCount(); i++) {
            model.removeRow(i);
            model.setRowCount(0);
        }
        try {
            ResultSet res = dbSearch(teams);
    ...
    public void addItem(int id, String status, String date, String teams) throws SQLException {
        String adding = "INSERT INTO attend values("+id+", '"+date+"', '"+status+"', '"+teams+"')";
        Statement stm = con.createStatement();
        stm.executeUpdate(adding);
    }

    public boolean check(String teams, String dt) throws SQLException {
        String str1 = "select * from attend where class = '"+teams+"' AND dt = '"+dt+"'";
        Statement stm = con.createStatement();
        ResultSet rst = stm.executeQuery(str1);
        if(rst.next())
            return true;
